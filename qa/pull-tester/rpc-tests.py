...
# The repository root, derived from this file's location (qa/pull-tester/), so
# the runner works from any working directory. `.exe` on Windows matches what
# the binaries are named there.
...
def main():
    ...
    if EXEEXT == ".exe" and not args.force:
    ...
    if args.help:
        ...
        parser.print_help()
        subprocess.check_call((REPOROOT + '/qa/rpc-tests/' + test_list[0]).split() + ['-h'])
        sys.exit(0)
...
class RPCTestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie zebrads, we can apply a
