...
#
# Scenario A2: retail, one mid-sized note holding an EXACT balance.
#
# A separate faucet wallet funds the subject with exactly 15 ZEC in one Orchard
# note (the faucet keeps its coinbase change, so the subject holds precisely
# 15 ZEC). The subject migrates to completion, exercising the common consumer
# case. The balance checks are inline and EXACT so a reviewer confirms from this
# file alone that a real 15-ZEC holder migrated its 15 ZEC into Ironwood.
#
...
class RetailScenario(IronwoodMigrationScenario):
    ...
    def run_test(self):
        ...
        assert_equal(len(source_notes), 1, "a single mid-sized source note")
