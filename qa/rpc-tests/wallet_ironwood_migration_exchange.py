...
#
# An exchange hot wallet holds its balance across many notes rather than one.
# This scenario funds the account and then splits it into many distinct Orchard
# notes, so the migration's preparation must consolidate a many-note source. It
# drives the migration to completion and asserts the exact balances inline: the
# Ironwood note count equals the plan's crossing count, every note is positive,
# and value is conserved (source = Ironwood + residual + a bounded fee).
#
...
# Split the funded balance into ten extra 5-ZEC notes, giving a many-note source.
SPLIT_NOTES = [5] * 10
...
class ExchangeScenario(IronwoodMigrationScenario):
    ...
    def run_test(self):
        ...
        print("Funding an Orchard balance and splitting it into a many-note "
              "shape...")
        ...
        assert_true(len(source_notes) >= 8,
                    "the source is a many-note wallet: {}".format(len(source_notes)))
