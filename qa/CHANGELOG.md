...
- A `wallet_z_send_scenarios.py` scenario reproducing the mining-pool payout bug (zcash/zallet#644) end to end on a fresh account: transparent-to-transparent payouts fragment the wallet's own funds onto internal change addresses, so the account reports a spendable total that no single-address source can pay in one transaction. The scenario drives the miner's coinbase -> shield -> de-shield pipeline, proves the fragmentation (change lands on a wallet-chosen address), pins the failure mode (naming a single address reports insufficient funds while the account balance covers the round), and then shows the `any_transparent` fund source paying the whole round in one transaction that gathers the internal change fragment and spends no shielded notes (the exchange-compatibility guarantee). Adds a `transparent_input_addresses` helper to `util.py` that resolves each transparent input's funding address from the node.
